...
class CorrectEquationSolver:
    ...
    def create_sensitivity_matrix(self, num_measurements: int, irradiation_type: str = 'long') -> np.ndarray:
        ...
        for i in range(num_measurements):
            for j in range(self.num_groups):
                ...
                # Физическая модель из уравнений
                # N(E_n) = Σ [A * (a_i / λ_i) * (1 - e^(-λ_i * t_irr)) * (e^(-λ_i * t_decay)) * (1 - e^(-λ_i * Δt)) * T_i] * χ_i(E_n)
                
                # Коэффициент A (нормировка)
                A_coeff = 1.0
                ...
                sensitivity = A_coeff * (a_i / lambda_i) * irradiation_factor * decay_factor * collection_factor * T_factor
                
                A[i, j] = sensitivity
            
            # Нормализуем строку (сумма коэффициентов = 1)
            row_sum = np.sum(A[i, :])
            if row_sum > 0:
                A[i, :] = A[i, :] / row_sum
    ...
    def solve_equations(self, measurements: np.ndarray, irradiation_type: str = 'long') -> Tuple[np.ndarray, np.ndarray]:
        ...
        for bin_idx in range(num_energy_bins):
            ...
            x_solution, residuals, rank, s = np.linalg.lstsq(A_matrix, N_measured, rcond=None)
            
            # Применяем физические ограничения
            x_solution = np.maximum(x_solution, 0)  # неотрицательность
            
            # Вычисляем неопределенности
            if len(residuals) > 0:
                # Оценка неопределенности на основе остатков
                uncertainty = np.sqrt(residuals[0] / (num_measurements - self.num_groups)) * np.ones(self.num_groups)
            else:
