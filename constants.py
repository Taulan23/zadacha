...
EXPERIMENT_PARAMS = {
    ...
    # Время распада после облучения (в секундах) - нижняя граница интервала измерения
    'decay_time': 0.12,
    
    # Время измерения (в секундах) - разница между верхней и нижней границей
    'measurement_time': 0.88,  # 1.0 - 0.12 = 0.88
    
    # Период измерения (в секундах) - общее время измерения
